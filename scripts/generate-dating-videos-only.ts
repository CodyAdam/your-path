...
async function main() {
  ...
  console.log("=== Dating Video Generation (videos only, keyframes already done) ===\n");
  ...
  for (const node of dating.nodes) {
    ...
    try {
      ...
      keyframeDataUris[node.id] = `data:image/png;base64,${buf.toString("base64")}`;
    } catch {
  ...
  console.log(`  ✅ node-01 main (${((Date.now() - node01Start) / 1000).toFixed(1)}s)`);
  ...
  console.log(`  ✅ Shared idle (${((Date.now() - idleStart) / 1000).toFixed(1)}s)\n`);
  ...
  const tasks = remainingNodes.map((node) => async () => {
    ...
    try {
      ...
      console.log(`  ✅ [${done}/${remainingNodes.length}] "${node.title}" — ${((Date.now() - nodeStart) / 1000).toFixed(1)}s`);
    } catch (err) {
      done++;
      const msg = err instanceof Error ? err.message : String(err);
      console.error(`  ❌ [${done}/${remainingNodes.length}] "${node.title}" — ${msg}`);
      failed.push(node.title);
    }
  });

  console.log(`Step 2: Generating ${remainingNodes.length} main videos (5 concurrent, 12s each)...\n`);
  await runWithConcurrency(tasks, 5);

  const totalSucceeded = (done - failed.length) + 1;
  const totalTime = ((Date.now() - startTime) / 1000 / 60).toFixed(1);
  console.log(`\n=== Complete (${totalTime} min) ===`);
  console.log(`  Videos: 1 idle + ${totalSucceeded}/${dating.nodes.length} main`);
