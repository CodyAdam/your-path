...
const ALLOWED_TYPES = ["audio/webm", "audio/mp3", "audio/mpeg", "audio/wav", "audio/mp4", "audio/x-m4a"];

export async function analyseAudio(formData: FormData): Promise<AnalyseAudioResult> {
  ...
  const type = (audio instanceof File ? audio.type : (audio as Blob).type)?.toLowerCase() || "";
  if (type && !ALLOWED_TYPES.some((t) => type === t || type.includes(t.replace("audio/", "")))) {
    throw new Error(`Unsupported audio type: ${type}. Allowed: ${ALLOWED_TYPES.join(", ")}`);
  }
